<script lang="ts">
	...
	import { Home, Search, LogOut, User, PlusSquare, MessageCircle, Eye, EyeOff } from 'lucide-svelte';
...
<div
	class="pointer-events-none fixed bottom-0 left-0 right-0 z-50 flex w-full flex-col items-center justify-end pb-2 md:pb-4"
>
	<div class="pointer-events-auto relative flex flex-col items-center">
		...
		<button
			...
			class="absolute bottom-0 flex h-10 items-center justify-center rounded-full border border-white/10 bg-black/50 px-4 text-neutral-400 shadow-[0_8px_30px_rgb(0,0,0,0.5)] backdrop-blur-md transition-all duration-300 hover:text-white {isDockHidden ? 'translate-y-0 opacity-100 z-10' : 'translate-y-8 opacity-0 -z-10 pointer-events-none'}"
		>
			<Eye class="mr-2 h-4 w-4" /> <span class="text-sm font-medium">Show Nav</span>
		</button>

		<!-- Main Dock -->
		<div class="transition-all duration-300 {isDockHidden ? 'translate-y-24 opacity-0 pointer-events-none' : 'translate-y-0 opacity-100'}">
			<Dock>
				<a href="/feed">
				<DockItem>
					<DockIcon slot="icon">
